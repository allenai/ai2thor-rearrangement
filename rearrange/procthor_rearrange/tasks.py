...
class UnshuffleTask(AbstractRearrangeTask, BaseUnshuffleTask):
    def __init__(
        self,
        sensors: SensorSuite,
        ...
        object_ids_seen_in_walkthrough: Set[str] = None,
        ...
        AbstractRearrangeTask.__init__(
            self,
            env=unshuffle_env,
            sensors=sensors,
            task_info=dict(),
        ...
        self.object_ids_seen_in_walkthrough = object_ids_seen_in_walkthrough
        ...
        self.greedy_expert: Optional[GreedyExploreUnshuffleExpert] = None
        self.actions_taken = []
    ...
    def query_expert(self, **kwargs) -> Tuple[Any, bool]:
        if self.greedy_expert is None:
            if not hasattr(self.unshuffle_env, "shortest_path_navigator"):
                self.unshuffle_env.shortest_path_navigator = ShortestPathNavigatorProcTHOR(
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            **AbstractRearrangeTask.metrics(self),
            **{
        ...
        if action_name.startswith("pickup"):
            ...
            with include_object_data(self.unshuffle_env.controller):
                ...
                obj_name_to_goal_and_cur_poses = {
                    cur_pose["objectId"]: (goal_pose, cur_pose)
                ...
                for o in self.unshuffle_env.last_event.metadata["objects"]:
                    if (
                        ...
                        and not self.unshuffle_env.are_poses_equal(
                            *obj_name_to_goal_and_cur_poses[o["objectId"]]
                        )
                    ):
    ...
    def step(self, action: int) -> RLStepResult:
        step_result = AbstractRearrangeTask.step(self, action=action)
...
class WalkthroughTask(AbstractRearrangeTask, BaseWalkthroughTask):
    def __init__(
        self,
        sensors: SensorSuite,
        walkthrough_env: RearrangeProcTHOREnvironment,
    ...
    def metrics(self, force_return: bool = False) -> Dict[str, Any]:
        ...
        metrics = AbstractRearrangeTask.metrics(self)
...
class RearrangeTaskSpecIterable:
    ...
    @property
    def task_specs_for_current_scene(self) -> List[RearrangeTaskSpec]:
        dicts = [
            compress_pickle.loads(b, compression="gzip")
            for b in self.task_spec_dicts_for_current_scene
        ]

        for d in dicts:
            if "scene" not in d:
                d["scene"] = self.current_scene
            else:
                assert self.current_scene == d["scene"]
        return [RearrangeTaskSpec(**d) for d in dicts]
    ...
    def __next__(self) -> RearrangeTaskSpec:
        ...
        new_task_spec_dict = compress_pickle.loads(
            self.task_spec_dicts_for_current_scene.pop(), compression="gzip"
        )
...
class RearrangeTaskSampler(TaskSampler):
    def __init__(
        ...
        self.walkthrough_env = RearrangeProcTHOREnvironment(**rearrange_env_kwargs)

        self.unshuffle_env: Optional[RearrangeProcTHOREnvironment] = None
        if self.run_unshuffle_phase:
            self.unshuffle_env = RearrangeProcTHOREnvironment(**rearrange_env_kwargs)
        ...
        **init_kwargs,
        ...
        scenes_to_task_spec_dicts = cls._filter_scenes_to_task_spec_dicts(
            scenes_to_task_spec_dicts=cls.load_rearrange_data_from_path(
                stage=stage, base_dir=STARTER_DATA_DIR, scenes=allowed_scenes
            ),
        ...
        return cls(
            ...
            expert_exploration_enabled=expert_exploration_enabled,
            **init_kwargs,
        )
        ...
        if (
            ...
            try:
                ...
                self.walkthrough_env.controller.step(
                    action="SetObjectFilter", objectIds=[]
                )
            ...
            self._last_sampled_task = UnshuffleTask(
                ...
                object_ids_seen_in_walkthrough=copy.copy(
